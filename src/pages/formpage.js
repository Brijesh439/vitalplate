...
const FormPage = () => {
    ...
    const [mockCustomers, setMockCustomers] = useState([
      ...
      { id: 2, name: 'Jane Smith', historySaved: true, nutritionSaved: false, historyData: {'name' : 'john doe', 'age': 23} },
      { id: 3, name: 'Mike Johnson', historySaved: true, nutritionSaved: true, historyData: {'name' : 'john doe', 'age': 23}, nutritionData: {'protein' : 12, 'carbs': '256 Kcal'}  },
    ]);
    ...
    const handleGenerateNutrition = () => {
        ...
        setMockCustomers(customers =>
            customers.map(customer =>
                customer.id === selectedCustomer?.id
                    ? { ...customer, nutritionSaved: true, nutritionData: generatedNutritionData }
                    : customer
            )
        );
    };
                ...
                <div style={halfWidthStyle}>
                    ...
                    {historyFormSubmitted && (
                        <NutrientForm
                            ...
                            onCreateMeals={() => { /* Handle meal creation logic here */ }}
                        />
                    )}
                </div>
            </div>
